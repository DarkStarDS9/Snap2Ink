#!/usr/bin/env bash
# Deploy Snap2Ink to TestFlight (Release scheme).
# Run from anywhere — paths are derived from this script's location.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT="$SCRIPT_DIR/Snap2Ink.xcodeproj"
...
: "${SNAP2INK_ASC_KEY_ID:?Set SNAP2INK_ASC_KEY_ID (see docs/testflight.md)}"
: "${SNAP2INK_ASC_ISSUER_ID:?Set SNAP2INK_ASC_ISSUER_ID (see docs/testflight.md)}"
ASC_KEY_ID="$SNAP2INK_ASC_KEY_ID"
ASC_ISSUER_ID="$SNAP2INK_ASC_ISSUER_ID"
ASC_KEY_PATH="${SNAP2INK_ASC_KEY_PATH:-$HOME/.appstoreconnect/private_keys/AuthKey_${ASC_KEY_ID}.p8}"
ARCHIVE_PATH=/tmp/Snap2Ink.xcarchive
EXPORT_PATH=/tmp/Snap2Ink-export
ARCHIVE_LOG=/tmp/xcodebuild-archive-snap2ink.log
EXPORT_LOG=/tmp/xcodebuild-export-snap2ink.log
...
diagnose_archive() {
    ...
    if grep -q "errSecInternalComponent" "$log"; then
        ...
        err "No provisioning profile found for app.snap2ink.camera."
        ...
        echo "  provisioningProfiles entry for the expected name: 'Snap2Ink AppStore')."
    elif grep -q "requires a provisioning profile" "$log"; then
        err "Missing provisioning profile (manual signing)."
        echo "  Check that 'Snap2Ink AppStore' is installed in:"
...
diagnose_export() {
    ...
    if grep -q "25 uploads" "$log" || grep -q "maximum number of builds" "$log" || grep -q "too many builds" "$log"; then
        ...
        err "No App Store Connect app record exists yet for app.snap2ink.camera."
        echo "  Create it once via the web UI: App Store Connect → My Apps → + → New App"
        echo "  (bundle ID app.snap2ink.camera is already registered — just attach it there)."
    else
...
xcodebuild archive \
    ...
    2>&1 | tee "$ARCHIVE_LOG" | grep -E "^(=== BUILD|=== ARCHIVE|.*error:|.*warning:.*profile|.*CodeSign|PhaseScriptExecution|CompileSwift)" | grep -v "^$" || true
...
xcodebuild -exportArchive \
    ...
    -authenticationKeyID "$ASC_KEY_ID" \
    -authenticationKeyIssuerID "$ASC_ISSUER_ID" \
    2>&1 | tee "$EXPORT_LOG" | grep -E "(Upload|Export|error:|Uploading|Done|No profiles|FAILED)" | grep -v "^$" || true
